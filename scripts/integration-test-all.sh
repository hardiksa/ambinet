...
## default port prefixes for ambinetd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/ambinetd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/ambinetd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/ambinetd add-genesis-account \
    "$("$PWD"/build/ambinetd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aambinet,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/ambinetd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/ambinetd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/ambinetd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/ambinetd start --pruning=nothing --rpc.unsafe \
    ...
    AMBINET_PID=$!
    echo "started ambinet node, pid=$AMBINET_PID"
    # add PID to array
    arr+=("$AMBINET_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test ambinet node $HOST_RPC ..."
