...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
AMBINET_BINARY = ambinetd
AMBINET_DIR = ambinet
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/hardiksa/ambinet.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := hardiksahq
PROJECT := ambinet
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=ambinet \
...
build-docker:
	...
	$(DOCKER) rm ambinet || true
	# create a new container from the latest image
	$(DOCKER) create --name ambinet -t -i ${DOCKER_IMAGE}:latest ambinet
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp ambinet:/usr/bin/ambinetd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/hardiksa/ambinet/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/hardiksa/ambinet
.PHONY: format
...
ETHERMINT_URL      	= https://raw.githubusercontent.com/hardiksa/ethermint/v0.10.0
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(AMBINET_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\ambinet\Z ambinetd/node "./ambinetd testnet --v 4 -o /ambinet --keyring-backend=test --ip-addresses ambinetdnode0,ambinetdnode1,ambinetdnode2,ambinetdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(AMBINET_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/ambinet:Z ambinetd/node "./ambinetd testnet --v 4 -o /ambinet --keyring-backend=test --ip-addresses ambinetdnode0,ambinetdnode1,ambinetdnode2,ambinetdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\ambinetd:ambinet\Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\ambinetd:ambinet\Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\ambinetd:ambinet\Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\ambinetd:ambinet\Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/ambinetd:/ambinet:Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/ambinetd:/ambinet:Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/ambinetd:/ambinet:Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/ambinetd:/ambinet:Z ambinetd/node "./ambinetd unsafe-reset-all --home=/ambinet"
endif
...
.PHONY: build-docker-local-ambinet localnet-start localnet-stop

###############################################################################
###                                Releasing                                ###
###############################################################################

PACKAGE_NAME:=github.com/hardiksa/ambinet
