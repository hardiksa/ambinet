...
1. Either [open](https://github.com/hardiksa/ambinet/issues/new/choose)
   or [find](https://github.com/hardiksa/ambinet/issues) an issue you have identified and would like to contribute to
   resolving.
...
3. If you would like to contribute:
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the ambinet repo, to ensure a
...
some [good first issues](https://github.com/hardiksa/ambinet/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Ambinet, please create
an [ADR](https://github.com/hardiksa/ambinet/blob/main/docs/architecture/README.md) so further discussions can be
...
at `https://github.com/hardiksa/ambinet`, the code should never exist
at `$GOPATH/src/github.com/hardiksa/ambinet`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/hardiksa/ambinet`, and do all the work there.

For instance, to create a fork and work on a branch of it, you would:

1. Create the fork on github, using the fork button.
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/hardiksa/ambinet`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:hardiksa/ambinet.git`

Now `origin` refers to my fork and `upstream` refers to the ambinet version. So I can `git push -u origin master` to
update my fork, and make pull requests to ambinet from there. Of course, replace `hardiksa` with your git handle.
...
4. Create your feature branch from `development` either on `github.com/hardiksa/ambinet`, or your fork (
...
**Note**: hardiksa’s Ambinet team currently cuts releases on a need to have basis. We will announce a more
