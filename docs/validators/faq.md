...
- **Validator's Address**: `ambinetvaloper1-` address. This is the address used to identify your validator publicly. The private key associated with this address is used to bond, unbond, and claim rewards.
...
ambinetd tx staking create-validator
    --pubkey ambinetvalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
    --amount "2aambinet"
    --from tmp
...
- **Tendermint Key**: This is a unique key used to sign block hashes. It is associated with a public key `ambinetvalconspub`.
    - Generated when the node is created with `ambinetd init`.
    - Get this value with `ambinetd tendermint show-validator`
    e.g. `ambinetvalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`

- **Application keys**: These keys are created from the application and used to sign transactions. As a validator, you will probably use one key to sign staking-related transactions, and another key to sign oracle-related transactions. Application keys are associated with a public key `ambinetpub-` and an address `ambinet-`. Both are derived from account keys generated by `ambinetd keys add`.

::: warning
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `ambinetvaloper` and `ambinetvaloperpub`
:::
...
If you want to obtain coins for the testnet, you can do so by using the [faucet](https://faucet.ambinet.dev/).
...
- **Transaction fees:** Ambinet maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `ambinet`.
