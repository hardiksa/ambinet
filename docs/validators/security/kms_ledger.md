...
- Create a `~/.tmkms/tmkms.toml` file with the following content (use an adequate `chain_id`)
  ...
  chain_id = "ambinet_9001-1"
  reconnect = true                                # true is the default
  secret_key = "~/.tmkms/secret_connection.key"

  [[providers.ledger]]
  chain_ids = ["ambinet_9001-1"]
  ```

- Edit `addr` to point to your `ambinetd` instance.
- Adjust `chain-id` to match your `.ambinetd/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `ambinetd`.
...
07:28:24 [INFO] [keyring:ledger:ledger] added validator key ambinetvalconspub1zcjduepqy53m39prgp9dz3nz96kaav3el5e0th8ltwcf8cpavqdvpxgr5slsd6wz6f
07:28:24 [INFO] KMS node ID: 1BC12314E2E1C29015B66017A397F170C6ECDE4A
```

The KMS may complain that it cannot connect to `ambinetd`. That is fine, we will fix it in the next section.
This output indicates the validator key linked to this particular device is: `ambinetvalconspub1zcjduepqy53m39prgp9dz3nz96kaav3el5e0th8ltwcf8cpavqdvpxgr5slsd6wz6f`
Take note of the validator pubkey that appears in your screen. *We will use it in the next section.*

## Ambinet configuration

You need to enable KMS access by editing `.ambinetd/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `ambinetd`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell ambinetd the key that we've got in the previous section.

```bash
ambinetd gentx --name kmsval --pubkey <pub_key>
```

Now start `ambinetd`. You should see that the KMS connects and receives a signature request.
