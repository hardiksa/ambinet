...
For information on how to use `ambinetd` binary to submit an on-chain proposal through the governance module, please refer to the [quickstart](../quickstart/binary.md) documentation.

### CLI

This is the command format for using `ambinetd` (the command-line interface) to submit your proposal on-chain:

```bash
ambinetd tx gov submit-proposal \
  --title=<title> \
  --description=<description> \
  --type="Text" \
  --deposit="1000000aambinet" \
  --from=<mykey> \
  --chain-id=<chain_id>
  --node <address>
```
...
::: tip
Use the `ambinet tx gov --help` flag to get more info about the governance commands
:::

1. `ambinetd` is the command-line interface client that is used to send transactions and query Ambinet
...
5. `--fees` is a flat-rate incentive for a validator to process your transaction
   ...
   - 7500aambinet is equal to 0.0075 AMBINET
6. `--chain-id ambinet_90001-2` is Ambinet Mainnet. For current and past chain-id's, please look at the [hardiksa/mainnet resource](https://github.com/hardiksa/mainnet)
   - the testnet chain ID is [ambinet_9000-4](https://testnet.mintscan.io/ambinet). For current and past testnet information, please look at the [testnet repository](https://github.com/hardiksa/testnets)
7. `--node` is using a full node to send the transaction to the Ambinet Mainnet

### Verifying your transaction

After posting your transaction, your command line interface (`ambinetd`) will provide you with the transaction's hash, which you can either query using ambinetd or by searching the transaction hash using [Mintscan](https://www.mintscan.io/ambinet) or any block explorer.
...
```bash
ambinetd q gov proposals --depositor ambinet1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
ambinetd tx gov deposit <proposal-id> <deposit> --from <name>
```

In our case above, the `<proposal-id>` would be 59 as queried earlier.
The `<deposit>` is written as `500000aambinet`, just like the example above.
...
- the deposit denomination is in 'atambinet' instead of 'aambinet'
