...
```bash
ambinetd keys add mykey

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(ambinetd keys show mykey -a)
```
...
yes $KEYPASSWD | ambinetd keys add me
yes $KEYPASSWD | ambinetd keys show me
# start ambinetd with keyring-backend flag
ambinetd --keyring-backend=file start
```
